!function(){
	var bP={};	
	var b=20, bb=600, height=2200, buffMargin=8, minHeight=14;
	var c1=[-200, 40], c2=[-70, 250], c3=[-5, 300]; //Column positions of labels.
	...
	function drawHeader(header, id){
		d3.select("#"+id).append("g").attr("class","header").append("text").text(header[2])
			.style("font-size","40").attr("x",358).attr("y",-10).style("text-anchor","middle")
		...
		data.forEach(function(biP,s){
			...
			[0,1].forEach(function(p){			
				d3.select("#"+biP.id)
					...
					.on("click", function(d, i){ return bP.clicked(data, p, i);})
	...
	bP.selectSegment = function(data, m, s){
		data.forEach(function(k){
			...
			selectedBar.select(".barlabel").style('font-weight','bold').style("cursor", "pointer");
			selectedBar.select(".barvalue").style('font-weight','bold');
			selectedBar.select(".barpercent").style('font-weight','bold');
		});
	}	
	
	bP.clicked = function(data, m, s){
		data.forEach(function(k){
			if(m ==1)
			{	
				var selectedBar = d3.select("#"+k.id).select(".part"+m).select(".mainbars")
				...
				selectedBar.select(".barvalue").style('font-weight','bold');
				selectedBar.select(".barpercent").style('font-weight','bold');
				var org = selectedBar.select(".barlabel").text();	
				window.location.href = "topcoder?org="+org;
			}
		});
	}
